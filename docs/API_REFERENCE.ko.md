...
- 지원 명령(기본):
  ...
  - `status` : 봇 상태
  - `help` : 도움말
  - `매수 KRW-BTC 100000` : 시장가 매수
  - `매수 KRW-BTC 10%` : 잔고 비율로 시장가 매수
  - `매수 KRW-BTC 100000 지정가 50000000` : 지정가 매수
  - `매도 KRW-BTC 0.01` : 시장가 매도(수량)
  - `매도 KRW-BTC 10%` : 보유 비율로 시장가 매도
  - `매도 KRW-BTC 0.01 지정가 50000000` : 지정가 매도
  - `미체결 내역` : 미체결 내역 조회
  - `미체결 내역 KRW-BTC` : 특정 마켓 미체결 내역
  - `체결 내역` : 체결 내역만 조회
  - `체결 내역 KRW-BTC` : 특정 마켓 체결 내역
  - `취소 내역` : 취소 내역만 조회
  - `취소 내역 KRW-BTC` : 특정 마켓 취소 내역
  - `취소 <UUID>` : 주문 취소(2단계 확인)
  - `확인 <토큰>` : 매수/매도/취소 주문 확정
  ...
  - 지정가 매수 수량은 계산 후 소수 8자리로 내림
  - 주문/체결/취소 내역 응답에 요약 1줄 + 상세(상태/가격/수량/거래대금/수수료/시간/uuid) 포함
